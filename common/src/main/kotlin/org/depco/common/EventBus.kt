package org.depco.common


import java.util.concurrent.ConcurrentHashMap
import kotlin.reflect.KClass

open class PrEvent(public val name:String = "PrEvent")
typealias Handler<T> = (e:T)->Unit

object EventBus{
    val mslog = Log("EventBus")
    val handlerMap = ConcurrentHashMap<KClass<*>,ArrayList<Handler<*>>>()
    inline fun <reified T: PrEvent>register(noinline handler: Handler<T>){
    ...
    inline fun <reified T: PrEvent>unRegister(noinline handler: Handler<T>){
        val handlers = handlerMap[T::class]
            ?: return
        handlers.remove(handler)
        mslog.info("handler size for ${T::class}"+handlers.size)
    }
    inline fun <reified T: PrEvent>post(event:T){
